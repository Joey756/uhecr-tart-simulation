The coreas_to_hdf5.py python script converts the standard coreas output into a HDF5 based data format. 
In addition to ALL radio related output, also the longitudional profile (*.long) and the main shower 
parameters from the *.inp file are saved in the HDF5 file. If the script is called with the optional 
option --highLevel, high-level radio quantities such as the energy fluence are calculated.
